...
import {
  exportPayrollSlipsWorkbook,
  type PayrollSlipRecord,
} from "@/lib/payroll/export-payroll-slips";
...
function employeeNo(record: PayrollRecord, index: number) {
  const employee = employeeFromRecord(record);
...
function toPayrollSlipRecord(
  ...
  return {
    ...
    employeeNo: employeeNo(record, index),
    salaryType: record.salary_type,
    hourlyRate: toNullableNumber(record.hourly_rate),
    monthlySalary: toNullableNumber(record.monthly_salary),
    nightRate: toNullableNumber(record.night_rate),
...
export async function GET(
  ...
  const records = ((data ?? []) as PayrollRecord[]).map((record, index) =>
    toPayrollSlipRecord(record, yearMonth, index),
  );
  ...
  const workbookBuffer = await exportPayrollSlipsWorkbook(yearMonth, records);
  const filename = `payroll-slips-${yearMonth}.xlsx`;
